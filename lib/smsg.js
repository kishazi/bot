...
function smsg(marcusInstance, message, store) {
  if (!message) {
    return message;
  }
  ...
  if (message.key) {
    ...
    message.sender = marcusInstance.decodeJid(
      message.fromMe && marcusInstance.user.id || message.participant || message.key.participant || message.chat || ''
    );
    
    if (message.isGroup) {
      message.participant = marcusInstance.decodeJid(message.key.participant) || '';
    }
  }
  ...
  if (message.message) {
    ...
    if (quotedMessage) {
      ...
      message.quoted.sender = marcusInstance.decodeJid(message.msg.contextInfo.participant);
      message.quoted.fromMe = message.quoted.sender === marcusInstance.decodeJid(marcusInstance.user.id);
      ...
      message.quoted.getQuotedObj = message.quoted.getQuotedMessage = async () => {
        ...
        let quotedMsg = await store.loadMessage(message.chat, message.quoted.id, marcusInstance);
        return exports.smsg(marcusInstance, quotedMsg, store);
      };
      ...
      message.quoted["delete"] = () => marcusInstance.sendMessage(message.quoted.chat, { 'delete': quotedMessageFakeObj.key });
      message.quoted.copyNForward = (toChat, forceForward = false, options = {}) => marcusInstance.copyNForward(toChat, quotedMessageFakeObj, forceForward, options);
      message.quoted.download = () => marcusInstance.downloadMediaMessage(message.quoted);
    }
  }

  // Handle URL in the message
  if (message.msg.url) {
    message.download = () => marcusInstance.downloadMediaMessage(message.msg);
  }
  ...
  message.reply = (replyText, replyTo = message.chat, options = {}) => {
    if (Buffer.isBuffer(replyText)) {
      return marcusInstance.sendMedia(replyTo, replyText, "file", '', message, { ...options });
    }
    return marcusInstance.sendText(replyTo, replyText, message, { ...options });
  };

  // Function to copy the message
  message.copy = () => exports.smsg(marcusInstance, messageInfo.fromObject(messageInfo.toObject(message)));

  // Function to forward the message
  message.copyNForward = (toChat = message.chat, forceForward = false, options = {}) => marcusInstance.copyNForward(toChat, message, forceForward, options);
